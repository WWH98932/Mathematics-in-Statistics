...
## 1.(a)
I choose the 1980-2017 GDP, Unemployment Rate and Inflation Rate(by CPI) annual data of the US, China and Japan, put them in a single csv document.
```{r}
setwd("D:/Econ 403A/Homework 2")
data1<-read.csv("countrydata.csv")
print(data1)
```
First of all, we should define the function of "marginal_plot"(https://github.com/ChrKoenig/R_marginal_plot).
```{r}
...
```
Generate the scatter plot with marginal densities.
```{r}
# GDP with Unemployment rate
marginal_plot(x=GDP,y=LUR,group=Country,data=data1,bw="nrd",xlab="GDP",ylab="LUR",pch=15,cex=0.5)

# GDP with Inflation
marginal_plot(x=GDP,y=CPI,group=Country,data=data1,bw="nrd",xlab="GDP",ylab="CPI",pch=15,cex=0.5)

# Unemployment rate with Inflation
marginal_plot(x=LUR,y=CPI,group=Country,data=data1,bw="nrd",xlab="LUR",ylab="CPI",pch=15,cex=0.5)
```

## 1.(b)
I will choose the GDP, LUR and CPI of 2016 as an example:
```{r}
# Conditional Distribution of GDP and Country 
data1[c(1,38,75),]
cdplot(Country~GDP,data=data1)
# Conditional Distribution of LUR and Country 
data1[c(1,38,75),]
cdplot(Country~LUR,data=data1)
# Conditional Distribution of CPI and Country 
data1[c(1,38,75),]
cdplot(Country~CPI,data=data1)
```
![](http://chart.googleapis.com/chart?cht=tx&chl=f(x,y)=cx(y-x)e^{-y}, 0\leq x\leq y\leq 1)
